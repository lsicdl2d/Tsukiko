...
from config.bot_config import signin_max_get_balance, signin_min_get_balance, signin_continued_reward, \
    signin_continued_reward_day
...
class Signin(User):
    def signinDatabaseInit(self):
        self.executeWithCommit(
            "CREATE TABLE IF NOT EXISTS `signin` (`qid` CHAR(12) NOT NULL,`timestamp` DECIMAL(38,6) NOT NULL,`continuedSignin` SMALLINT NOT NULL,`totalSignin` SMALLINT NOT NULL,`todaySignin` TINYINT NOT NULL,`lastReward` DECIMAL(38,2) NOT NULL, PRIMARY KEY ( `qid` )) ENGINE=InnoDB DEFAULT CHARSET=utf8;")

    def userSigninInit(self, qid):
        self.executeWithCommit(
            f"INSERT INTO signin (qid, timestamp, continuedSignin, totalSignin, todaySignin, lastReward) VAlUES ({qid}, {time.time()},0,0,0,0);")
    ...
    def setContinuedSignin(self, qid):
        ...
        if self.checkContinuedSignin(qid):
            self.executeWithCommit(
                f"UPDATE signin SET continuedSignin={self.getContinuedSignin(qid) + 1} WHERE qid='{qid}';")
        else:
    ...
    def signin(self, qid):
        ...
        if not self.checkTodaySignin(qid):
            ...
            uconomy.setUserBalance(self.getSteamId(qid),
                                   uconomy.getUserBalance(self.getSteamId(qid)) + Decimal(reward))  # 打钱
            if self.getContinuedSignin(qid) % signin_continued_reward_day == 0 and self.getContinuedSignin(
                    qid) != 0:  # 连续签到x日特别打钱
                uconomy.setUserBalance(self.getSteamId(qid), uconomy.getUserBalance(self.getSteamId(qid)) + Decimal(
                    signin_continued_reward))  # 打钱
